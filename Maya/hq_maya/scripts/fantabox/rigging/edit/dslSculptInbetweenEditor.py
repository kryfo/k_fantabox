...
reload(dslDo)
reload(dslRs)
#get_tool_rig = fb.rigging.edit.__path__
#icon_path=("\\\\10.99.1.13\\hq_tool\\Maya\\hq_maya\\scripts\\fantabox\\rigging\\edit\\dslSculptInbetweenEdit_icons\\")
icon_path=cmds.getModulePath(moduleName='hq_maya')+'/scripts/fantabox/rigging/edit/dslSculptInbetweenEdit_icons/'
#===============================================================================
# GLOBAL VARIABLES
#===============================================================================
...
class SculptInbetweenEditor():
    ...
    def ui(self):
        ...
        if cmds.window('dsl_Sculpt_Inbetween_Editor', title=self.winName, ex=True):
            ...
            crFormLayout = cmds.formLayout(width=150, parent=self.mainPanelLayout)
            ...
            def textDataTypeChange(type):
                if cmds.radioButtonGrp('radioGrpDataOption', q=True, select=True) == 1:
                    ...
                    cmds.formLayout('fieldFromTo', e=True, enable=True)
                ...
                if cmds.radioButtonGrp('radioGrpDataOption', q=True, select=True) == 3:
                    ...
                    cmds.formLayout('fieldFromTo', e=True, enable=False)
            #=================================================================== 
            ...
            toleranceText = cmds.text('TolText', label='Tolerance',parent='fieldFromTo')
            ...
            cmds.formLayout('fieldFromTo', 
                            ...
                            af=[('fromCr', 'left', 0),
                                ...
                                (toleranceFactor, 'right', 10),
                                #(buttonClear, 'bottom', 0),
                                (toleranceText, 'bottom', 0),
                                (toleranceFactor, 'bottom', 0)
                                ],
                            ac=[
                                ('toCr', 'top', 5, 'fromCr'),
                                (buttonClear, 'top', 5, 'toCr'),
                                (toleranceText, 'top', 10, buttonClear),
                                (toleranceFactor, 'left', 10, toleranceText)
                                ])
            ...
            cmds.floatField('bellowVal', height=20, 
                            width=60, parent='formPruneFrame', 
                            min=0.001, value=0.01, step=0.01)
            cmds.separator('firstSep', st='in')
            cmds.text('textFactor', label='Factor:', enable=False, height=20,
                      parent='formPruneFrame')
            cmds.floatField('factorValue', enable=False, height=20, width=60, 
                            parent='formPruneFrame', step=0.01, 
    ...
    def listInbetween(self, inbetweenList=None,
        ...
        for idx, correctiveItem in enumerate(inbetweenList):
            ...
            self.slider = cmds.floatSliderButtonGrp(nameInbSlider,
                                                    ...
                                                    image=(icon_path+'dslCmDelete.xpm'),
                                                    minValue=0.000,
                                                    maxValue=1.000,
                                                    bgc=bcgValue,
    ...
    def listCorrective(self, corrective=None, arg=None):
        ...
        if checkState == True:
            for cr in listCorrective:
                if cmds.getAttr(blendShapeNode + '.' + cr) != 0:
